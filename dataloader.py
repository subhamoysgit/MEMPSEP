...
def dataLoader_r(file_list, batch_size, input_dict):
    '''
    dataLoader for MEMPSEP regression
    @author: Subhamoy Chatterjee
    Parameters:
        file_list (list): list of flare onset dates from training/validation
        batch_size (int): size of training batches
    ...
    #### peparing insitu parameter list   
    df = pd.read_csv(DATA_DIR+'all_ML_parameters_int_flux.csv',header=1)
    insitu_cols = ['SW Temp', 'SW Velocity', 'SW Density', 'IMF B', 
                   'IMF Bx', 'IMF By', 'IMF Bz', 'Fe/O Lo', 'Fe/O Hi',
                   'H Lo', 'H Hi', 'O Lo', 'O Hi', 'Fe Lo', 'Fe Hi']
    
    etype0 = list(df['event_type']==0)
    etype1 = list(df['event_type']==1)

    target_cols = ['sep_peak_1','sep_peak_2', 'sep_peak_3','sep_peak_4','sep_peak_5']
    for col in target_cols:
        ind = np.logical_and(list(df[col]==-9999),etype0)
        ind_c = np.logical_and(list(df[col]!=-9999),etype0)
        df.loc[ind,col] = df[col][ind_c].sample(np.sum(ind),replace=True,random_state=239).values
    ...
    target_cols_2 = ['FlrtoSEP_dt','sep_duration']
    ...
    df_target = df[target_cols+target_cols_2]
    #logarithm of SEP property targets
    df_target = np.log10(df_target)
    ...
    for col in insitu_cols:
        if col in ['IMF Bx','IMF By','IMF Bz']:
            ...
            df[col] = pd.DataFrame(scale.fit_transform(df[[col]].values), columns=[col], index=df.index)
    ...
    while True:
        ...
        for r in range(batch_size):
            ...
            if f_batch[r][:12] in dw:
                X2[r,:,:,0] = pickle.load(open(DATA_DIR+'wind_waves/ww_'+f_batch[r][:12]+'.p','rb'))
            X2[r,:,:,0] = input_dict['W']*X2[r,:,:,0]
            ...
            if f_batch[r][:12] in ds:
                xray = pd.read_csv(DATA_DIR+'x_ray/'+f_batch[r][:12]+'.csv',header=1)
                xray = xray.fillna(0)
                xray = xray[['0.45nm', '0.175nm']].to_numpy()
                for i in range(2):
                    if np.max(xray[:,i])>0: 
            ...
            if f_batch[r][:12] in de:
                elec = pd.read_csv(DATA_DIR+'electrons_L1/'+f_batch[r][:12]+'.csv')
                elec = elec.fillna(0)
                elec = elec[['fepm_E_0','fepm_E_1','fepm_E_2','fepm_E_3','fepm_E_4','fepm_E_5','fepm_E_6']].to_numpy()
                if elec.shape[0]>8640:
                    elec = elec[-8640:,:]
                for i in range(7):
                    if np.max(elec[:,i])>0: 
            ...
            Y[r,1:8] = np.asarray(df_target.iloc[idx,:].astype('float'))
